...
type UseDashboardShellModelInput = {
  role: AppRole;
};

export function useDashboardShellModel({ role }: UseDashboardShellModelInput) {
  const router = useRouter();
  const { showToast } = useAppToast();
  const [logout, logoutState] = useLogoutMutation();
  const dashboardSummary = useGetDashboardSummaryQuery();
